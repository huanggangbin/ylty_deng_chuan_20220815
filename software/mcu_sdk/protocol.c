...
void all_data_update(void)
{
    ...
    mcu_dp_enum_update(DPID_WORK_MODE,0); //枚举型数据上报;
    mcu_dp_value_update(DPID_BRIGHT_VALUE,0); //VALUE型数据上报;
    mcu_dp_value_update(DPID_TEMP_VALUE,0); //VALUE型数据上报;
    mcu_dp_string_update(DPID_COLOUR_DATA,0,0); //STRING型数据上报;
    mcu_dp_value_update(DPID_COUNTDOWN,0); //VALUE型数据上报;
    mcu_dp_raw_update(DPID_RGBIC_LINERLIGHT_SCENE,0,0); //RAW型数据上报;
    mcu_dp_raw_update(DPID_DREAMLIGHTMIC_MUSIC_DATA,0,0); //RAW型数据上报;
    mcu_dp_value_update(DPID_LED_NUMBER_SET,0); //VALUE型数据上报;
    mcu_dp_raw_update(DPID_PAINT_COLOUR_DATA,0,0); //RAW型数据上报;
...
/*****************************************************************************
函数名称 : dp_download_dreamlightmic_music_data_handle
功能描述 : 针对DPID_DREAMLIGHTMIC_MUSIC_DATA的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_dreamlightmic_music_data_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为RAW
    unsigned char ret;
    /*
    ...
    */
    ...
    //There should be a report after processing the DP
    ret = mcu_dp_raw_update(DPID_DREAMLIGHTMIC_MUSIC_DATA,value,length);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_paint_colour_data_handle
功能描述 : 针对DPID_PAINT_COLOUR_DATA的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_paint_colour_data_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为RAW
    unsigned char ret;
    /*
    ...
    */
    ...
    //There should be a report after processing the DP
    ret = mcu_dp_raw_update(DPID_PAINT_COLOUR_DATA,value,length);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
    ...
    ***********************************/
    ...
    switch(dpid) {
        ...
        case DPID_DREAMLIGHTMIC_MUSIC_DATA:
            //炫彩本地音乐律动处理函数
            ret = dp_download_dreamlightmic_music_data_handle(value,length);
        break;
        case DPID_LED_NUMBER_SET:
            //led点数设置处理函数
            ret = dp_download_led_number_set_handle(value,length);
        break;
        case DPID_PAINT_COLOUR_DATA:
            //点控处理函数
            ret = dp_download_paint_colour_data_handle(value,length);
        break;
